...
collegues did in their study in 2013, the intensity of indirect daily
...
detailed analysis. Four elements guided the analysis with the following
questions:
...
Due to the hilly topography and high elevations, wild boar are expected
to move to higher elevations on average in the summer months and then
down in the cooler months with potential snowfall. Areas with high
density of location points are assumed to be particular sources of food
for this boar such as corn fields for example. Concerning the use of the
landcover by this boar, the hypothesis is an important use of forest
spaces, especially during the day. Finally, in view of the region where
the boar is located, which is an area with many trails and anthropogenic
activities, it is assumed that the boar presents mainly a nocturnal
behavior. Hunting should have an influence on its movements and its use
of the landcover.
...
Nature and Forest departement based on the fact that the wild boar can
be predominantly nocturnal in areas with anthropic disturbance (Ohashi
et al., 2013 ; Podgorski et al., 2013) Location data could thus be
obtained until July 2021. The animal then suddenly disappeared and so
did the GPS transmission.
...
```{r, fig.cap="Fig 1: Map of the region",fig.align='center'}
...
and time were also merged together in a new column. a "Timelag" column
...
of the "Date" and "Time" data. A "Daytime" column has been added. This
one is based on the "Timelag" column. If the time difference is less
than 65 minutes, it concerns measurements taken at night (from 6pm to
6am), if this difference is greater than 360 minutes, it concerns
measurements taken during the day (between 6am and 6pm). The column
therefore contains the information Day or Night.
...
distance traveled in a certain time lapse. A "steplength" column was
created. This one indicates the distance between two rows, i.e. two
location points. Finally, a "Speed" column has been created. This one is
the result of the "Steplength" column divided by the "Timelag" column.
...
The hunting pressure category 3 corresponds only to two week in the
year.
...
Wild boars are territorial (Suter & Fischer, 2021). In order to
understand the extent of the male boar's territory in this study, a
visual analysis of its presence was performed. I performed a mcp derived
from the location points of this boar to identify its home range. For
this purpose, the function st_convex_hull has been applied. It takes a
point cloud as input and draws a convex hull around the most distant
vertices. The area was then calculated using the chull.area function and
compared to the results obtained on ArcGis Pro for validation. The home
range was analysed for all the data from 2019 to 2021
...
Density approach. Following this result, and in order to understand if
these areas correspond to areas where the wild boar stayed for a long
time or if it came back all the time, an analysis of the path by ggplot
with the geom_path function was performed for each month in 2020.
...
the year 2020 has been created. Not all parcels could be assigned to a
...
requiring detail such as a particular field, I worked with visualisation
...
In order to verify this, two subsets were created: one for the day and
one for the night. The average of the two subsets was calculated and
then they were compared. For a better visualization, a boxplot was made.
A day/night comparison was made by boxplot according to the months in
2020.

Finally, as speed can also depend on the type of surface on which the
boar moves (behaviour adapted to land cover), a boxplot analysis of the
...
The analysis of the altitude at which the boar were located from 2019 to
...
```{r, }
aggregate(Altitude~Month, FUN = mean, data = jojo)
```

Table 1 : Mean altitude for each month between 2019 and 2021.
...
```{r, fig.cap="Fig 4: Density of presence of the wild boar in 2020", fig.width=50, fig.align='center'}
...
the other months. In January 2020, about 20% of the wild boar were
located in areas not identified in the available geopackage compared to
10% in December 2020 (Figure 7).
...
the GIS layer provided by the Departement for agriculture did not
...
speed increased during the day and during the night (Table 3, Table 4).

```{r, fig.cap="Fig 12: Speed depending on the hunting pressure in 2019 and 2020", fig.align='center'}
...
show that the boar in this study had a territory of 158.67 m2 which is
...
correlation with the description in Suter and Graf 2021

general trend of wild boar to leave the natural habitats and their
margins between march and july (Gerard et al. 1991)

**Activity patterns**

favour natural habitats more marked during resting times than during
activity phases (Gerard et al. 1991)

Clear nocturnal activity due to anthropogenic disturbance (Ohashi et al.
2013 ; Podgorski et al. 2013).

**Effect of hunting**

nocturnal activity should increase when they are exposed to hunting
pressure (Keuling et al. 2008 ; Ohashi et al. 2013). Nocturnal activity
did not increase during hunting season (Brivio et al. 2017).
...
Thurfjelle H., Spong G. & Ericsson G. (2013) : Effects of hunting on
